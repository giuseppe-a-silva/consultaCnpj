...
const getData = sessionId =>
  request({
    ...
    jar: makeJar('http://www.receita.fazenda.gov.br/pessoajuridica/cnpj/cnpjreva/Cnpjreva_qsa.asp'),
    method: 'GET',
...
const convertValueToFloat = string =>
  parseFloat([
    unmasker(string).slice(0, (unmasker(string).length - 2)),
    unmasker(string).slice((unmasker(string).length - 2), unmasker(string).length),
  ].join('.'));
...
const parseExtractedDataFromBody = body => {
  ...
  data = extractDataCapitalSocial(base);
  if (processValue(data)) {
