...
const hasCapitalSocialInfo = (k, v, r) =>
  !(k.startsWith('CAPITAL SOCIAL:')
    && (r.test(v)
      || v.includes('Não informada')
      || !v));

const processCodeAndDescriptions = (s, r) =>
  ((r.test(s))
    ? ({
...
const parseExtractedDataFromBody = (body) => {
  ...
  if (hasCapitalSocialInfo(k, v, asterixRegex)) {
  ...
  base.each((i, elem) => {
    ...
    if (i % 2 === 0) {
      ...
      partner[k] = processCodeAndDescriptions(v, dashedStringRegex);
    }
  });
  result.QSA = partners;
