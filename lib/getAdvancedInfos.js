...
/**
 * Procura por frases que caracterizem erro no corpo do site.
 * @param  {String}  body Corpo do site.
 * @return {Promise}      Contendo o corpo do site ou erro.
 */
...
/**
 * [processMoneyToReal description]
 * @param  {[type]} string [description]
 * @return {[type]}        [description]
 */
function processMoneyToReal(string) {
...
/**
 * [processCapitalSocial description]
 * @param  {[type]} elem [description]
 * @return {[type]}      [description]
 */
function processCapitalSocial(elem) {
  const [key, value] = [
    elem
      .eq(0)
      .text()
  ...
  return processMoneyToReal(value);
}

/**
 * Transforma string do modelo X(...)X-X(...)X em objeto contando
 * código como primeiro trecho antes do "-" e descrição como segundo.
 * @param  {String}  string  X(...)X-X(...)X.
 * @return {Object}          Objeto contendo informações transformadas.
 */
...
/**
 * [parseExtractedDataFromBody description]
...
function parseExtractedDataFromBody(body) {
  ...
  result['CAPITAL SOCIAL'] = processCapitalSocial(base);
...
/**
 * [getAdvancedInfos description]
 * @param  {[type]} sessionId [description]
 * @return {[type]}           [description]
 */
function getAdvancedInfos(sessionId) {
  if (!sessionId) {
    return buildPromise(new Error('Valores informados são inválidos!'));
  }

  return getData('ASPSESSIONIDCCARBTDS=HJDODNJDHPIMHLAICNGMKJMJ')
